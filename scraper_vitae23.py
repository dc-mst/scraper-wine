...
def extract_winery_and_wine_names(content):
    ...
    for block in winery_blocks:
        ...
        for i, match in enumerate(matches):
            ...
            price_match = re.search(r'€\s?(\d+(\.\d{1,2})?)', block)
            ...
            # Extract bottle number
            bottle_match = re.search(r'Bt\. ([\d\.,]+)', block)
            if bottle_match:
...
def write_to_csv(entries):
    ...
    with open(f'static/pdf/vitae23-{region}.csv', 'w', newline='') as file:
        ...
        for winery_name, wine_name, vintage, wine_prefix, price, extracted_text, grapes, alcohol_content, bottle_number, maturation_details in entries:
            ...
            # Fixed values
            row[columns.index('RatingYear')] = '2023'
            ...
            row[columns.index('Tasting Notes')] = main_text if len(main_text) > 100 else None
...
with open(f'static/pdf/vitae23-{region}.txt', 'r') as file:
    content = file.read()
